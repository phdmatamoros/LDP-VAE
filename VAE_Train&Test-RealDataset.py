...
filetxt = open('ResultsVotes.txt', "a+")
filetxt.write("Epoch Patience latent Space FlipBIT Attribute Min_loss"+'\n')
filetxt.close()
##########################################
data_lab=pd.read_csv('csv_sep/Votes_label_server.csv') 
data_hash=pd.read_csv('csv_sep/Votes_hash_server.csv')
        ...
        for mask_ind in range(0,len(checkdat)):
            ...
            for epoch in range(1000):#1000
                for x, y in train_dl:
                    ...
                    loss = loss_fn(recon, y, z_loc, z_scale)
                    #print(loss)
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                ...
                with torch.no_grad():
                    for i, (x, y) in enumerate(test_dl):
                        ...
                        z = vae.reparameterize(z_loc, z_scale)
                        recon = vae.decoder(z,len_string)
                        test_loss = loss_fn(recon, y, z_loc, z_scale)
                ...
                if total_epoch_loss_test.item() < 0.000001+np.min(loss_test_history):
                    ...
                    torch.save(vae.state_dict(), 'results/'+names+'_'+fptracad+'_'+cadena+"_VAE_model.pt")
                else:
                    patience +=1
            ...
            vae =[]        
            vae = VAE()
            vae.load_state_dict(torch.load('results_09/'+names+'_'+fptracad+'_'+cadena+"_VAE_model.pt"))
            vae.eval()
            with torch.no_grad():
            ...
            for times_random in Randomtimes:
                ...
                outdfb.to_csv("results/Votes"+LS+names+'_flopprob_'+str(fptra)+'_peratt_'+str(times)+'_'+'probabilities_centroid.csv')
                #outdfc=[]
